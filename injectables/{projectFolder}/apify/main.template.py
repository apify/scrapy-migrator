...
t(f"from {spider_module_name} import {spider_class_name}")
...
def _get_scrapy_settings(max_depth: int) -> Settings:
    ...
    settings['ITEM_PIPELINES'][t(f"'{apify_module_path}.pipelines.ActorDatasetPushPipeline'")] = 1
    ...
    settings['DOWNLOADER_MIDDLEWARES'][t(f"'{apify_module_path}.middlewares.ApifyRetryMiddleware'")] = 999
    # Add our custom Scheduler
    settings['SCHEDULER'] = t(f"'{apify_module_path}.scheduler.ApifyScheduler'")
...
async def main():
    async with Actor:
        ...
        t(f"process.crawl({spider_class_name})")
        process.start()
